...
def job():
    ...
    for record in data['records']['data']:
        for STRIKE_PRICE in constants.STRIKE_PRICE_YOU_WANT:
            if record.get("expiryDate") == constants.EXPIRY and record.get(
    ...
    # ======================== NEW =====================
    DIFF = TOTAL_PE_COI - TOTAL_CE_COI

    if TOTAL_CE_COI < 0:
        PCR = round(abs(DIFF / TOTAL_CE_COI), 2)
    elif TOTAL_PE_COI < 0:
        PCR = round(abs(TOTAL_PE_COI / DIFF), 2)
    else:
    ...
    timestamp_str = ist_time.strftime('%H:%M:%S')

    # Sample data to write

    headers = [
        'TIMESTAMP', 'TOTAL_CE_COI', 'TOTAL_PE_COI', 'DIFF', 'PCR', 'SIGNAL'
    ]
    data = [timestamp_str, TOTAL_CE_COI, TOTAL_PE_COI, DIFF, PCR, FINAL_SIGNAL]
...
def print_data_on_console():
    if constants.HEADER_PRINTED is False:
        ...
        with open(constants.FILENAME, 'r', newline='') as csvfile:
            ...
            print(tabulate([new_row], headers=[],
                           tablefmt='grid'))  # No header repeat, simple line


def wait_until_9_18_and_run(func):
    ...
    secs_to_wait = (datetime.combine(now_naive.date(), target_time) -
                    now_naive).total_seconds()
    if secs_to_wait > 0:
        print(f"Waiting for {secs_to_wait} seconds until 9:18 AM...")
